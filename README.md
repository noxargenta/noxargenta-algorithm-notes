...
| [📌 模板收藏](#-模板收藏) | 经典模板题汇总 |
| [⚡ 你代码里的套路总结](#-你代码里的套路总结) | 从你的代码中提炼的常用写法和小技巧 |

---

## 📖 学习路径

按算法竞赛常见的学习顺序组织，每个知识点标注了你的练习文件。
...
deque<int> q;
for(int i = 0; i < n; i++) {
    if(!q.empty() && q.front() <= i - k) q.pop_front();  // 移除窗口外
...
```cpp
const int INF = 0x7f7f7f7f;       // int 范围哨兵
const int INFLL = 0x3f3f3f3f3f3f3f3f; // long long 范围哨兵
