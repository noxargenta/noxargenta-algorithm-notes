# 算法笔记

刷题过程中积累的常用写法和技巧，方便速查。

| # | 笔记 | 来源 |
|---|------|------|
| 01 | [输入输出格式化](./01-输入输出格式化.md) — setfill/setw/setprecision | lanqiao2 |
| 02 | [二分答案模板](./02-二分答案模板.md) — 二分 + check 函数 | D_Jumping_Through_Segments |
| 03 | [并查集 DSU](./03-并查集DSU.md) — 路径压缩 + 扩展域 | F_Cow_and_Snacks / K_团伙 |
| 04 | [单调栈与单调队列](./04-单调栈与单调队列.md) — NGE + 滑窗 | P_5788 / P_1886 |
| 05 | [差分数组技巧](./05-差分数组技巧.md) — 一维/二维差分 | Meximum_Array_2 |
| 06 | [图论 DFS 与 BFS 模板](./06-图论DFS与BFS模板.md) — memset/方向数组/马走日BFS/FloodFill/岛屿淹没 | P1443 / P1596 / 全球变暖 / P_9872 |
| 07 | [常用常数与宏](./07-常用常数与宏.md) — INF / 基础模板 / 快读 | 通用 |
| 08 | [树形DP：树上括号方案数](./08-树形DP-树上括号方案数.md) — 平衡度状态转移 | MC0560 |
| 09 | [BFS常见错误与调试](./09-BFS常见错误与调试.md) — 1-based/边界/标记已访问 | 走迷宫 |
| 10 | [质数筛法与判定](./10-质数筛法与判定.md) — O(√n)试除 / 欧拉线性筛 / bitset | P_1036 |
| 11 | [背包DP](./11-背包DP.md) — 01背包(倒序) vs 完全背包(正序) / 一维优化 | P_1048 / P_1060 / P_1616 |
| 12 | [线性DP：LIS与网格路径](./12-线性DP-LIS与网格路径.md) — LIS二重循环/贪心二分/Dilworth/过河卒 | P_1091 / P_1020 / P_1002 |
| 13 | [lower_bound / upper_bound 用法](./13-lower_bound-upper_bound用法.md) — LIS变体/降序greater/坐标压缩/set二分 | P_1020 + 各文件 |
| 14 | [线性基 Linear Basis](./14-线性基.md) — 插入/求最大XOR/两集合XOR和最大化 | Bitwise_Maximization (2025牛客多校2 B) |
| 15 | [状压DP — 排列计数](./15-状压DP-排列计数.md) — 拓扑排序计数/满足偏序约束的排列数 | Lazy_Shuffling (2025牛客多校2 L) |
| 16 | [stringstream 用法](./16-stringstream用法.md) — 按空格分词/数字字符串互转/自定义分隔符 | D_Mail_Institution_Codes (2025牛客多校7 D) |
| 17 | [异或找两个奇数次数 + Brian Kernighan](./17-异或与BrianKernighan.md) — 异或抵消 + 提取最右 1 分组 | Two_Odd_Count_Numbers (左程云030_5) |
| 18 | [C++ 标准库技巧](./18-C++标准库技巧.md) — complex几何/hypot/clamp/lerp/midpoint/gcd/lcm/内置位运算 | 左程云系列 + 整理 |
| 17 | [取模等式计数 — CRT与打表找规律](./17-取模等式计数-CRT与打表找规律.md) — 合法 a 为连续区间 / O(√n) 整除计数 | Bobo_s_Lucky_Modulo (2025牛客多校7 L) |
